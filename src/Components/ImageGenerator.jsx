...
const ImageGenerator = () => {
    ...
    return(
        <div className="AI_Image_Generator">
            <div className="Header text-slate-50 text-center text-2xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl font-black pt-7">AI Image <span className="generatorSpan text-sky-400">Generator</span></div>
            ...
            <div className="img_div grid grid-cols-2 w-2/5 gap-5 m-auto sm:grid md:grid lg:grid lg:grid-cols-2 xl:grid xl:grid-cols-2 2xl:grid 2xl:grid-cols-2 xxs:grid xxs:grid-cols-1 xxs:w-3/4 xs:w-3/4 sm:w-7/12 md:w-6/12 lg:w-5/12 xs:grid xs:grid-cols-1 sm:grid-cols-1 xxs:mb-16 xs:mb-10 sm:mb-16 md:grid-cols-1 md:mb-4 lg:mb-4 xl:mb-4 2xl:mb-4">
                    {image_url.map((image_urls, index) => (
                        <img key={index} src={image_urls === "/" ? default_image : image_urls} className="imageGen w-full h-44 sm:h-full md:h-48" alt="" />
                    ))}
            </div>

            <div className="loading">
                    <div className={loading?"loading-text flex justify-center mb-3 text-xs":"display-none hidden"}>Loading...</div>
                    <div className={loading?"loading-bar-full mb-3 w-12/12 h-1 bg-white m-auto duration-10000":"loading-bar w-0"}></div>
            </div>
        </div>
        </div>
    )
